...
namespace AstronomicalCalculationLib
{
    public static class AstronomicalCalculator
    {
        ...
        public static double CalculateGravity(double mass, double radius)
        {
            ...
            if (radius <= 0)
            {
                throw new AstronomicalCalculationArgumentException("Radius parameter must be a positive value", radius.ToString(), nameof(radius));
            }
        ...
        public static double CalculatePlanetGravity(string planetName)
        {
            if (planetName == null)
            {
                throw new AstronomicalCalculationArgumentException("Planet name is null", planetName, nameof(planetName));
            }
            if (planetName == string.Empty)
            {
                throw new AstronomicalCalculationArgumentException("Planet name is empty", planetName, nameof(planetName));
            }
            if (!Enum.TryParse<Planets>(planetName, out Planets planetEnum))
            {
                throw new InvalidPlanetArgumentException(planetName);
            }
